...
export function activate(context: vscode.ExtensionContext) {
    ...
    const disposableBuildAll = vscode.commands.registerCommand(COMMANDS.sayHello1, async () => {
        await buildAll({ useFilters: true });
    });
    const disposable2 = vscode.commands.registerCommand(COMMANDS.sayHello2, async () => {
        await buildCurrentFile({ useFilters: true });
    });
...
async function buildAll(opt: { useFilters: boolean }) {
...
async function buildCurrentFile(opt: { useFilters: boolean }) {
    ...
    const isFreezed: boolean = text.includes(".freezed.dart");
    const isG: boolean = text.includes(".g.dart");
